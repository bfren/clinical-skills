...
namespace Domain.Commands.UpdateDefaultClinicalSetting.UpdateDefaultClinicalSettingHandler_Tests;

public sealed class HandleAsync_Tests : Abstracts.UpdateSettings.HandleAsync_Tests
{
	internal sealed class TestHandler : TestHandlerBase<UpdateDefaultClinicalSettingCommand, UpdateDefaultClinicalSettingHandler, ClinicalSettingId>
	{
		internal sealed class Setup : SetupBase
		{
			public Setup() : base("Clinical Setting") { }

			internal override UpdateDefaultClinicalSettingCommand GetCommand(AuthUserId? userId = null, ClinicalSettingId? itemId = null) =>
				new(userId ?? LongId<AuthUserId>(), LongId<UserSettingsId>(), Rnd.Lng, itemId);

			internal override UpdateDefaultClinicalSettingHandler GetHandler(Vars v) =>
