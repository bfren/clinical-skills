...
namespace Domain.Queries.GetTrainingGrade.GetTrainingGradeHandler_Tests;

public sealed class HandleAsync_Tests : Abstracts.GetSingle.HandleAsync_Tests
{
	internal sealed class TestHandler : TestHandlerBase<ITrainingGradeRepository, TrainingGradeEntity, TrainingGradeId, GetTrainingGradeQuery, GetTrainingGradeHandler, TrainingGradeModel>
	{
		internal sealed class Setup : SetupBase
		{
			internal Setup() : base(true, "Training Grade") { }

			internal override GetTrainingGradeHandler GetHandler(Vars v) =>
				new(v.Cache, v.Repo, v.Log);

			internal override GetTrainingGradeQuery GetQuery(AuthUserId? userId = null, TrainingGradeId? entityId = null)
			{
				...
				return new(LongId<AuthUserId>(), LongId<TrainingGradeId>());
			}

			internal override TrainingGradeModel NewModel { get; } =
				new(LongId<AuthUserId>(), LongId<TrainingGradeId>(), Rnd.Lng, Rnd.Str, Rnd.Flip);
		}
	}
	...
	{
		await new TestHandler.Setup().Test00<Messages.TrainingGradeIdIsNullMsg>((h, q) => h.HandleAsync(q));
	}
	...
	{
		await new TestHandler.Setup().Test04<Messages.TrainingGradeDoesNotBelongToUserMsg>((h, q) => h.HandleAsync(q));
	}
