...
namespace Domain.Queries.GetClinicalSettings.GetClinicalSettingsHandler_Tests;

public sealed class HandleAsync_Tests : Abstracts.GetEnumerable.HandleAsync_Tests
{
	internal sealed class TestHandler : TestHandlerBase<IClinicalSettingRepository, ClinicalSettingEntity, ClinicalSettingId, GetClinicalSettingsQuery, GetClinicalSettingsHandler, ClinicalSettingsModel>
	{
		internal sealed class Setup : SetupBase
		{
			public Setup() : base("Clinical Settings") { }

			internal override GetClinicalSettingsHandler GetHandler(Vars v) =>
				new(v.Repo, v.Log);
			...
			internal override ClinicalSettingsModel NewModel { get; } =
				new(LongId<ClinicalSettingId>(), Rnd.Str);
		}
	}
