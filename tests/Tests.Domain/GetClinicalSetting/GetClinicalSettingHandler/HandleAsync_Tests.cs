...
using Domain.GetClinicalSetting.Messages;
using Jeebs.Auth.Data;
using Persistence.Entities;
using Persistence.Repositories;
using Persistence.StrongIds;

namespace Domain.GetClinicalSetting.GetClinicalSetting_Tests;
...
public sealed class HandleAsync_Tests : Abstracts.GetSingle.HandleAsync_Tests
{
	...
	{
		await new TestHandler.Setup().Test00<ClinicalSettingIdIsNullMsg>((h, q) => h.HandleAsync(q));
	}
	...
	{
		await new TestHandler.Setup().Test04<ClinicalSettingDoesNotBelongToUserMsg>((h, q) => h.HandleAsync(q));
	}
