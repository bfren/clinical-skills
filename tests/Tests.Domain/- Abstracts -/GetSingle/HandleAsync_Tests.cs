...
public abstract class HandleAsync_Tests
{
	...
	internal abstract class TestHandlerBase<TRepo, TEntity, TId, TQuery, THandler, TModel> : TestHandlerBase<TRepo, TEntity, TId, THandler>
		...
		where TModel : IWithUserId, IWithId<TId>
	{
		...
		{
			protected SetupBase(bool enableCache, string name) : base(enableCache, name) { }
		}
			...
			where TVars : Vars, new()
		{
			...
			protected SetupBase(bool enableCache, string name) =>
				(EnableCache, Name) = (enableCache, name);

			internal override (THandler handler, TVars v) GetVars()
			{
